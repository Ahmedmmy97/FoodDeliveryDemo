...
class SushiFragment : Fragment() , SwipeableFrameLayout.onSwipeEventDetected {


    private lateinit var binding: HomeFragmentBinding
    private val viewModel: HomeViewModel by activityViewModels()
    private val cats = arrayListOf(Category("Sushi"), Category("Drinks"), Category("Pizza"));
            ...
            override fun onClick(category: Category) {
                category.title!!.let {
                    if (it.equals(cats[1].title)){
                        val action = SushiFragmentDirections.actionSushiFragmentToDrinksFragment()
                        findNavController().navigate(action)
                    }else if (it.equals(cats[2].title)){
                        val action = SushiFragmentDirections.actionSushiFragmentToHomeFragment()
                        findNavController().navigate(action)
                    }
                }
            }
    ...
    override fun swipeLeftDetected() {
        if (viewModel.isToolBarCollapsed){
            val action =
                SushiFragmentDirections.actionSushiFragmentToDrinksFragment()
            findNavController().navigate(action)
        }
